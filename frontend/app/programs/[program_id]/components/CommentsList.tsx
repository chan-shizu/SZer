...
export default async function CommentsList({ programId }: { programId: number }) {
  ...
  return (
    <ul className="space-y-4 divide-y divide-border">
      {sorted.length === 0 && <li className="text-muted-foreground text-center py-8">まだコメントはありません</li>}
      {sorted.map((c) => (
        <li key={c.id} className="flex items-start gap-3 pb-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-semibold text-foreground text-sm">{c.user_name ? c.user_name : "未ログイン"}</span>
              <span className="text-xs text-muted-foreground">{new Date(c.created_at).toLocaleString()}</span>
            </div>
            <div className="text-foreground text-base break-words whitespace-pre-line">{c.content}</div>
          </div>
        </li>
      ))}
    </ul>
